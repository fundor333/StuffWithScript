...
class Scrape:
    ...
    def main(self):
        ...
        keys = []
        ids = ['Spot-Coffee-Elmwood',
               'eastloshigh']  # if a page name contains multiple words, separated by space (e.g. SPOT Coffee); it will show up as words connected by hyphens in URL. For example, SPOT Cofee as in "Spot-Coffee-Elmwood." If so, it is recommended that you use page id. You can find page id in the URL - it is the string of numbers after page name in URL.  (e.g. https://www.facebook.com/pages/Spot-Coffee-Elmwood/316579834919)
        for feed in ids:
            kid = feed
            d = get_data(kid)
