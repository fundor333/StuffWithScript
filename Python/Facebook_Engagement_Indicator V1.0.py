...
import dateutil.parser as dateparser # This is for converting ISO-8601 formatted date information (returned by Facebook Graph API) https://developers.facebook.com/docs/reference/api/dates/
...
# All following columns need to existant already in the DB, for any tables called in the script, you need to define here.
    ...
    def __init__(self, id, feed_id, last_comment, date_inserted,content, status_link, content_full, link, date, author, 
        ...
        time_since_post_14days, like_count_14days, comment_count_14days, share_count_14days,content_cycle,content_cycle_new,
        ):
...
for row in all_statuses:
    ...
    if time_since_post.days > 7 and not row.like_count_7days:
        ...
        if test:
            ...
            if content:
                content_new = content.replace('&#x27;', "'").replace('&amp;', "&").replace('&quot;', "'").replace('&amp', "&")
               ...
               if content:
                   ...
                   content_new3 = content_new3.replace('&#x27;', "'").replace('&amp;', "&").replace('&quot;', "'").replace('&amp', "&")
